...
def validate_experiment_dataDirectory(dm_experiment_name: str):
    ...
    logger.info('validate_experiment_dataDirectory starts here.')
...
def dm_api_ds():
    ...
    logger.info('dm_api_ds starts here.')

    dm_source_environ()
    logger.info('Service URL = %s', DsApiFactory.getServiceUrl())
    logger.info('OS environ = %s', environ['DM_DS_WEB_SERVICE_URL'])
...
def dm_start_daq(experimentName: str, dataDirectory: str, **daqInfo):
    ...
    daqInfo *dict*:
        ...
        See https://git.aps.anl.gov/DM/dm-docs/-/wikis/DM/Beamline-s/API-Reference/DAQ-Service#dm.daq_web_service.api.experimentDaqApi.ExperimentDaqApi.startDaq
        for details.
